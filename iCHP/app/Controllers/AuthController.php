...
class AuthController extends BaseController
{
     ...
     public function __construct()
     {
          ...
          $this->mongo = new \MongoDB\Client("mongodb://dev:ppso--@10.0.0.8:27017/?authSource=iSAS");
          $this->dbiSAS = $this->mongo->selectDatabase("iSAS");
     }
     ...
     private function randomPassword(int $length = 8)
     {
      ...
      //TODO [POST] /auth/login    check username with LDAP
      public function login()
     {
          try {
               ...
               if (!$validate) {
                    $server    = "snc-former.com"; 
                    $user      = "$Username"."@snc-former.com";
                    $firstname = "$Username";

                    return $this->response->setJSON($server); 
                    
                    // connect to active directory
                    $connectLDAP = ldap_connect($server);                          // ทำการเชื่อมต่อ server LDAP => Lightweight Directory Access Protocol 
                    ...
                    if (!$connectLDAP) {
                         ...
                         $checkUser = @ldap_bind($connectLDAP, $user, $Password);   // bind สำหรับยืนยันตัวตนกับ server ส่ง parameter ในนี้มาเช็ค
                         if(!$checkUser) {
                              die ('<p style="color:red;">warning</p>');
                         } else {      // ผ่านการเช็ค user และเชื่อมต่อสำเร็จให้ทำงานตามโค้ดด้านล่าง
                              ...
                              $results = $this->dbiSAS->selectCollection("Accounts")->find($filter, $options);
                              ...
                              $this->dbiSAS->selectCollection("Accounts")->updateOne($filter, ['$set' => $update]);
                         ...
                         // echo 'ลงชื่อเข้าใช้ LDAP สำเร็จ';
                    }
               }
     ...
     //TODO [POST] /auth/login  -- ตัวเก่าใช้งานได้แบบ ไม่ต้องเช็ค user กับ เว็บกลาง --   
     // public function login()
     // {
     //      try {
     ...
     //                ]
     //           ];
     //           $result = $this->dbiSAS->selectCollection("Accounts")->find($filter, $options);
     ...
     //                     ]
     //                ];
     //           $result = $this->dbiSAS->selectCollection("Accounts")->aggregate($pipeline);
     ...
     //           $this->dbiSAS->selectCollection("MailNotifyForgotPassword")->insertOne($data);
